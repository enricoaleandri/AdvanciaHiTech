...
class lavoriDAO extends AbstractDAO
{
    ...
    private $getAllLavori = "select * from ! as g,(	select max(id_lavoro ), id_lavoro_parent as idd from ga_lavori as f 	where f.deleted = 0 	and f.id_lavoro_parent <> -1 	group by f.id_lavoro_parent ) as d where g.id_lavoro_parent = -1 and g.id_lavoro <> d.idd UNION select * from ga_lavori as j,(	select max(id_lavoro ) as id , id_lavoro_parent as idd from ga_lavori as f  where f.deleted = 0  and f.id_lavoro_parent <> -1  	group by f.id_lavoro_parent 	) as k where k.id = j.id_lavoro";
    ...
    function updateLavoroByID(LavoroBean $lavoroBean)
    {
        ...
        if($this->connection->getReady())
        {
            ...
            {
                Logger::log(Logger::$ERROR, "updateLavoroByID  - Eccezione in  : $en");
            }
    ...
    function deleteLavoroById($id){
        if($this->connection->getReady())
        {
            ...
            {
                Logger::log(Logger::$ERROR, "deleteLavoroById - Eccezione in deleteLavoroById : $en");
            }
    ...
    function publishLavoroById($id){
        if($this->connection->getReady())
        {
            ...
            {
                Logger::log(Logger::$ERROR, "publishLavroById - Eccezione in publishLavroById : $en");
            }
